            ...
            _.each($scope.program.todos, function (todo) {
                if (todo.questId != null) {
                    if ($scope.quest.id == todo.questId) {
                        ...
                        /********
                        Fox for Title change and republish will not erase user data.
                        Need to check to see if Todo Feedback, Verification and Media already exist. if not add if needed. 
                        **********/

                        //verify all verification have questions
                        if (validate(false)) {
                            var feedbackChallengeExist = false;
                            var mediaChallengeExist = false;
                            //check to see if todo.challenges has media and feedback in challenges. if yes then do nothing. else add.
                            _.each(todo.challenges, function (challenge) {
                                switch (challenge.title) {
                                    case 'Todo Feedback':
                                        feedbackChallengeExist = true;
                                        break;
                                    case 'Todo Media Upload':
                                        mediaChallengeExist = true;
                                        break;
                                }
                            });

                            //save a feedback challenge if does not exist
                            if (!feedbackChallengeExist) {
                                var challenge = {
                                    type: 'general',
                                    title: 'Todo Feedback',
                                    instructions: null,
